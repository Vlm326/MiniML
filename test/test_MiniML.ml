...
let rec string_of_expr = function
  ...
  | Ast.If (cond, then_branch, else_branch) ->
      Printf.sprintf "If(%s,%s,%s)" (string_of_expr cond)
        (string_of_expr then_branch) (string_of_expr else_branch)
  ...
  [
    ...
    ( "helpers advance the lexbuf",
      ...
      fun () ->
        ...
        Alcotest.(check (option char)) "peek before get" (Some 'x')
          (Lexer.Lexer.peek peek_buf);
        Alcotest.(check (option char)) "get first char" (Some 'x')
          (Lexer.Lexer.get peek_buf);
        Alcotest.(check (option char)) "peek after get" (Some 'y')
          (Lexer.Lexer.peek peek_buf) );
  ]

let parser_tests =
  [
    ("precedence", `Quick, expect_parse "1 + 2 * 3" (Ast.BinOp (Ast.Add, Ast.Int 1, Ast.BinOp (Ast.Mul, Ast.Int 2, Ast.Int 3))));
    ("application associates left", `Quick, expect_parse "f x y" (Ast.App (Ast.App (Ast.Var "f", Ast.Var "x"), Ast.Var "y")));
    ("function application accepts let argument", `Quick, expect_parse "f (let x = 1 in x)" (Ast.App (Ast.Var "f", Ast.Let ("x", Ast.Int 1, Ast.Var "x"))));
    ("if expression", `Quick, expect_parse "if true then 1 else 2" (Ast.If (Ast.Bool true, Ast.Int 1, Ast.Int 2)));
    ("function literal", `Quick, expect_parse "fun x -> x + 1" (Ast.Fun ("x", Ast.BinOp (Ast.Add, Ast.Var "x", Ast.Int 1))));
    ("let rec with inline parameter", `Quick, expect_parse "let rec fact n = if n = 0 then 1 else n * fact (n - 1) in fact 4" (Ast.LetRec ("fact", "n", Ast.If (Ast.BinOp (Ast.Eq, Ast.Var "n", Ast.Int 0), Ast.Int 1, Ast.BinOp (Ast.Mul, Ast.Var "n", Ast.App (Ast.Var "fact", Ast.BinOp (Ast.Sub, Ast.Var "n", Ast.Int 1)))), Ast.App (Ast.Var "fact", Ast.Int 4))));
    ("let rec with fun keyword", `Quick, expect_parse "let rec inc = fun x -> x + 1 in inc 2" (Ast.LetRec ("inc", "x", Ast.BinOp (Ast.Add, Ast.Var "x", Ast.Int 1), Ast.App (Ast.Var "inc", Ast.Int 2))));
    ("boolean operators", `Quick, expect_parse "not false || true && false" (Ast.BinOp (Ast.Or, Ast.UnOp (Ast.Not, Ast.Bool false), Ast.BinOp (Ast.And, Ast.Bool true, Ast.Bool false))));
    ("trailing token is rejected", `Quick, expect_parse_failure "1 < 2 < 3" "Unexpected trailing token: LT");
    ("missing let name is rejected", `Quick, expect_parse_failure "let = 1 in 2" "Expected IDENT or REC");
    ("missing if then branch separator", `Quick, expect_parse_failure "if true 1 else 2" "Expected THEN, got ELSE");
    ("bare let rec value is rejected", `Quick, expect_parse_failure "let rec f = 1 in f" "let rec requires a function definition, use `let rec f x = ... in ...` or `let rec f = fun x -> ... in ...`");
    ("unexpected token in atom", `Quick, expect_parse_failure ")" "Unexpected token in expression: RPAREN");
  ]

let eval_tests =
  [
    ("curried addition", `Quick, expect_run "let add = fun x -> fun y -> x + y in add 3 4" "7");
    ("recursive factorial", `Quick, expect_run "let rec fact n = if n = 0 then 1 else n * fact (n - 1) in fact 5" "120");
    ("lexical scoping", `Quick, expect_run "let x = 10 in let f = fun y -> x + y in let x = 100 in f 5" "15");
    ("boolean equality", `Quick, expect_run "true = false" "false");
    ("boolean inequality", `Quick, expect_run "true <> false" "true");
    ("integer comparisons", `Quick, expect_run "1 < 2 && 2 <= 2 && 3 > 2 && 3 >= 3" "true");
    ...
    ("recursive function values stringify as fun", `Quick, expect_run "let rec id x = x in id" "<fun>");
    ("division by zero fails", `Quick, expect_failure "10 / 0" "you cannot divide by zero");
    ("if condition type error", `Quick, expect_failure "if 1 then 2 else 3" "Condition of if-expression must be bool, got int");
    ("applying non-function fails", `Quick, expect_failure "1 2" "Cannot apply value of type int as a function");
    ("unbound variable fails", `Quick, expect_failure "x" "Unbound variable: x");
    ("binary type mismatch on addition", `Quick, expect_failure "1 + true" "Type error in binary operation +: got int and bool");
    ("binary type mismatch on equality", `Quick, expect_failure "1 = false" "Type error in binary operation =: got int and bool");
    ("binary type mismatch on and", `Quick, expect_failure "1 && true" "Type error in binary operation &&: got int and bool");
    ("unary type mismatch", `Quick, expect_failure "not 1" "Type error in unary operation not: got int");
  ]
